 ...
 * @psalm-method ObjectCollection&\Traversable<ChildMigrations> find(ConnectionInterface $con = null) Return ChildMigrations objects based on current ModelCriteria
 * @method     ChildMigrations[]|ObjectCollection findById(int $id) Return ChildMigrations objects filtered by the id column
 * @psalm-method ObjectCollection&\Traversable<ChildMigrations> findById(int $id) Return ChildMigrations objects filtered by the id column
 ...
 * @psalm-method ObjectCollection&\Traversable<ChildMigrations> findByMigration(string $migration) Return ChildMigrations objects filtered by the migration column
 ...
 * @psalm-method ObjectCollection&\Traversable<ChildMigrations> findByBatch(int $batch) Return ChildMigrations objects filtered by the batch column
 ...
 * @psalm-method \Propel\Runtime\Util\PropelModelPager&\Traversable<ChildMigrations> paginate($page = 1, $maxPerPage = 10, ConnectionInterface $con = null) Issue a SELECT query based on the current ModelCriteria and uses a page and a maximum number of results per page to compute an offset and a limit
 *
 */
abstract class MigrationsQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'golden_clean', $modelName = '\\Migrations', $modelAlias = null)
    {
